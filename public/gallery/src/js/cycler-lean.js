//to do
//1 add a class 'container-small' to right hand column
//2. Create a json 'itemsextra.json' and test
...
(function(global){
  ...
  function ImageCycler() {
    ...
    this.containersmall = document.querySelector('.container-small');
  ...
  ImageCycler.prototype.getItems = function(callback) {
    ...
    request.open('GET', 'gallery/src/itemsextra.json', true);
    ...
    request.onload = (function() {
      if (request.status >= 200 && request.status < 400) {
        try{
          ...
          this.itemsextra = data.itemsextra;
          callback.call(this);
        } catch(e) {
  ...
  ImageCycler.prototype.render = function() {
    ...
    //thumbsextra
    var htmlextra
     for (var i = 0; i < this.itemsextra.length; i++) {
      htmlextra += 
        '<img src="'+ this.itemsextra[i].thumb + '"/>';
    }


    this.thumbs.insertAdjacentHTML('beforeend', html);
    this.thumbs.insertAdjacentHTML('beforeend', htmlextra);
